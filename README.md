        ...
        <div class="bg-gray-800 rounded-lg shadow-xl w-full max-w-sm p-8 border border-gray-700 text-center relative">
            ...
            <form id="loginForm">
                <div class="space-y-4 text-left mt-6">
                    <div><label for="username" class="block text-sm font-medium text-gray-300 mb-1">Username</label><input type="text" id="username" value="Rahul" required class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"></div>
                    <div><label for="password" class="block text-sm font-medium text-gray-300 mb-1">Password</label><input type="password" id="password" value="Rahul@123" required class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"></div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let state = {
                isAdmin: false,
                movies: [
                    { id: 1, title: 'Blade Runner 2049', posterUrl: 'https://placehold.co/500x750/000000/FFFFFF/png?text=Blade+Runner+2049', description: 'A young blade runner\'s discovery of a long-buried secret leads him to track down former blade runner Rick Deckard, who\'s been missing for thirty years.', category: 'Sci-Fi', year: 2017, downloadUrl: '#' },
                    { id: 2, title: 'Inception', posterUrl: 'https://placehold.co/500x750/111827/FFFFFF/png?text=Inception', description: 'A thief who steals corporate secrets through the use of dream-sharing technology is given the inverse task of planting an idea into the mind of a C.E.O.', category: 'Sci-Fi', year: 2010, downloadUrl: '#' },
                    { id: 3, title: 'The Dark Knight', posterUrl: 'https://placehold.co/500x750/1f2937/FFFFFF/png?text=The+Dark+Knight', description: 'When the menace known as the Joker wreaks havoc and chaos on the people of Gotham, Batman must accept one of the greatest psychological and physical tests of his ability to fight injustice.', category: 'Action', year: 2008, downloadUrl: '#' },
                    { id: 4, title: 'Parasite', posterUrl: 'https://placehold.co/500x750/374151/FFFFFF/png?text=Parasite', description: 'Greed and class discrimination threaten the newly formed symbiotic relationship between the wealthy Park family and the destitute Kim clan.', category: 'Thriller', year: 2019, downloadUrl: '#' },
                    { id: 5, title: 'Spirited Away', posterUrl: 'https://placehold.co/500x750/4b5563/FFFFFF/png?text=Spirited+Away', description: 'During her family\'s move to the suburbs, a sullen 10-year-old girl wanders into a world ruled by gods, witches, and spirits, and where humans are changed into beasts.', category: 'Animation', year: 2001, downloadUrl: '#' },
                    { id: 6, title: 'Mad Max: Fury Road', posterUrl: 'https://placehold.co/500x750/6b7280/FFFFFF/png?text=Mad+Max', description: 'In a post-apocalyptic wasteland, a woman rebels against a tyrannical ruler in search for her homeland with the help of a group of female prisoners, a psychotic worshiper, and a drifter named Max.', category: 'Action', year: 2015, downloadUrl: '#' },
                ],
                categories: ['Sci-Fi', 'Action', 'Thriller', 'Animation'],
                banners: [
                    { id: 1, imageUrl: 'https://placehold.co/1200x300/1e293b/4f46e5?text=Limited+Time+Offer!', targetUrl: '#' }
                ],
                myList: [2, 5],
                currentView: 'home',
                heroInterval: null,
            ...
            function renderView(view, filter = '') {
                ...
                if (view === 'home') {
                    ...
                    // Detail view is rendered separately
                }
            }
                ...
                return `
                    ...
                    <div id="movie-categories" class="p-4 sm:p-10 space-y-12">
                        ${state.categories.map(category => {
                            const moviesInCategory = groupedMovies[category] || [];
                            if (moviesInCategory.length === 0) return '';
                            return `
                                <section>
                                    <div class="flex justify-between items-center mb-6">
                                        <h2 class="text-2xl md:text-3xl font-bold text-gray-300">${category}</h2>
                                        ${moviesInCategory.length > 10 ? `<button data-action="view-more" data-category="${category}" class="text-indigo-400 hover:text-indigo-300 text-sm font-semibold">View More &rarr;</button>` : ''}
                                    </div>
            ...
            function renderCategoriesInModal() {
                $('#categoryList').innerHTML = state.categories.map(cat => `
                    <div class="flex items-center justify-between bg-gray-700 p-2 rounded-md">
                        ...
                        <div class="space-x-2">
                            <button data-action="edit-cat" data-name="${cat}" class="text-blue-400 hover:text-blue-300">Edit</button>
                            <button data-action="delete-cat" data-name="${cat}" class="text-red-400 hover:text-red-300">Delete</button>
                        </div>
                    </div>
                `).join('');
            }
                ...
                if (actionButton) {
                    ...
                    switch (action) {
                        ...
                        case 'edit':
                            ...
                            break;
                        case 'delete':
                            showConfirm('Delete Movie', 'Are you sure you want to delete this movie?', () => {
                        ...
                        case 'edit-cat':
                            ...
                            $('#editCategoryId').value = name;
                            $('#categorySubmitBtn').textContent = 'Update';
                            break;
                        case 'delete-cat':
                            if (state.movies.some(m => m.category === name)) {
                                alert('Cannot delete a category that contains movies. Please reassign or delete the movies first.');
                                return;
                            }
                            showConfirm('Delete Category', `Are you sure you want to delete the "${name}" category?`, () => {
                        ...
                        case 'edit-banner':
                            ...
                            break;
                        case 'delete-banner':
                             showConfirm('Delete Banner', 'Are you sure you want to delete this banner?', () => {
                    ...
                    renderDetailView(parseInt(movieCard.dataset.movieId));
                } else if (heroButton) {
                    renderDetailView(parseInt(heroButton.dataset.movieId));
                }
            }
            ...
            function handleMovieFormSubmit(e) {
                ...
                const movieData = {
                    title: $('#title').value, posterUrl: $('#posterUrl').value, downloadUrl: $('#downloadUrl').value,
                    description: $('#description').value, category: $('#category').value, year: parseInt($('#year').value)
                };
                if (id) {
            ...
            function handleCategoryFormSubmit(e) {
                ...
                const oldName = $('#editCategoryId').value;
                ...
                if (oldName) {
                    ...
                    state.movies.forEach(m => { if(m.category === oldName) m.category = newName; });
                } else {
            ...
            function handleBannerFormSubmit(e) {
                e.preventDefault();
                const id = parseInt($('#editBannerId').value);
                const bannerData = {
                ...
                if (id) {
                    ...
                    state.banners[index] = { ...state.banners[index], ...bannerData };
                } else {
